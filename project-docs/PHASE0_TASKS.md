...
- [x] **6.2a Delete-to-Trash — ALREADY SOLVED, better than the PRD assumed.** darktable has three distinct actions in `src/control/jobs/control_jobs.c`:
  ...
  - **Live-verified (2026-08-01)**: ran the actual built `darktable` binary against a scratch `--configdir`/`--library`, pointed at a folder containing `IMG_0001.CR3` + `IMG_0001.JPG` (synthetic stub files — real image decoding fails as expected, but that's irrelevant to import/grouping, which only depends on `dt_util_test_image_file()` passing and the filename pattern, per `src/common/utility.c:366-389` and `src/common/image.c:1822`). Directory args auto-import via `dt_film_import()` at startup with no confirmation dialog (`src/common/darktable.c:436-449`). Queried the resulting `library.db` directly: both rows landed with the same `group_id=1`, RAW (id=1) as representative, JPEG (id=2) grouped under it — exactly matching the source-level prediction. Confirms the import/grouping mechanism end-to-end, not just by code reading.
